...
export function AppHeader({ activeTab, onTabChange, onAccountClick }: AppHeaderProps) {
  ...
  return (
    <header className="border-b border-slate-200/80 bg-white/90 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-2 px-3 sm:px-5 lg:px-8">
        <Link className="flex shrink-0 items-center gap-2 sm:gap-3" to="/">
          ...
          <span>
            ...
            <small className="ml-1.5 hidden text-xs text-slate-400 sm:inline">ZhiLiao</small>
          </span>
        </Link>
        ...
        {activeTab && onTabChange ? (
          <nav
            ...
            aria-label="房间分类"
            style={{ display: "grid", gridTemplateColumns: "repeat(2, max-content)", flexShrink: 0 }}
          >
            {(["official", "private"] as const).map((tab) => (
              <button
                className={`whitespace-nowrap rounded-lg px-3 py-2 text-sm font-medium transition sm:px-5 ${
                  activeTab === tab
        ...
        <button
          className="flex shrink-0 items-center gap-3 rounded-xl px-2 py-1.5 text-left transition hover:bg-slate-100"
